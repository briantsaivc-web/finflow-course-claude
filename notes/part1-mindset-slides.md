...
| **疊代週期** | 改一版耗時以月為單位 | 5 天推進 49 次版本，至今 74 次 |
...
| **記帳訓練** | 紙筆手寫記帳，節奏拖沓冗長 | 記帳自動化三級制，隨熟練度解鎖 |
...
| **對手邏輯** | 3 種基礎 NPC 行為模式 | 作為數值平衡驗證的對照組 |
...
<div class="lead">動手寫程式碼前，先定三條不可動搖的基石。</div>

- **鐵律一：引擎與 UI 徹底分離**
  規則計算是引擎，畫面長相是外殼；改版面絕不准動到數值邏輯
- **鐵律二：資料驅動 (Data-Driven)**
  卡片數值全為獨立資料表，非寫死在邏輯中；增刪卡片免改引擎
- **鐵律三：每筆金錢變動必產生日記帳**
  每一塊錢流向皆有分錄可查，帳目對不上直接退回重寫
...
| 分級類別 | 判定標準 | 具體功能處置策略 |
| :--- | :--- | :--- |
| **現在做 (MVP)** | 沒有它，核心遊戲迴圈無法運作 | 職業基本盤、固定輪數、買賣結算 |
| **之後做 (Next)** | 有它更豐富，但沒有也能玩得通 | 技能樹深度、特殊事件、自動升級 |
...
<div class="grid">
  ...
  <div class="arrow">➔</div>
  <div class="box red">
    <strong>程式碼落地 (基線驗收)</strong><br>
    20 張職業全數解析入庫
  </div>
</div>

- 容許想法長大，但必須透過版本記錄逐一核對落差

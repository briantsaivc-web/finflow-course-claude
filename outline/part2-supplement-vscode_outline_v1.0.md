...
# 第二部補充（獨立小節）・同一件事的三種做法：命令提示字元、VS Code、瀏覽器裡的 VS Code（11 頁）
...
S2-2b. **實際長這樣（一）**——截圖兩張：Changes 清單（5 個 U、Commit 按鈕）／Staged Changes＋訊息已填
S2-2c. **實際長這樣（二）**——截圖：Sync Changes 1↑ 與「會 pull 再 push」對話框
...
S2-7b. **踩坑實況**——截圖兩張：「沒有 staged 變更，要全部收進來嗎」對話框／訊息空白跳出 COMMIT_EDITMSG；S2-7 表格另加兩列：訊息空著按 Commit、檔案顯示 M 卻看不到差異（CRLF，用 .gitattributes 統一 LF）
...
- S2-2b／2c／7b：截圖 `images/VSC-02`～`VSC-06`（Brian 電腦，2026-09-08，推本章那次的實際操作，已裁切到面板／對話框）
- S2-8：決策流程圖
...
## 待確認（Brian）
- S2-5 Restricted Mode 截圖是否要加框版；Source Control 面板截圖已於 v1.1 補齊（VSC-02～06）
- 放在第二部第 49 頁後（11 頁）或當 GitHub 章附錄
